...
import useColumns from '../search/useColumns.js';
...
/**
 * Ventas Ligne Roset — the monthly supplier sell-through report BODY (month
 * picker + export-and-send + the sell-through table), self-contained so it
 * renders both as its own page (LigneRosetSales) and inline as a lens of the
 * sales command screen (the Ventas workspace). Owns its queries + month state;
 * the host supplies only the page chrome (gate + header) when there is one.
 *
 * Lists our floor sales (accepted quotes not tied to an import order, recognized
 * at the deposit) for a chosen month, one row per product sold, and exports the
 * CSV + opens a prefilled email draft to Ligne Roset in one click. Defaults to
 * last month (what you send on the 15th).
 */
export default function LigneRosetReport() {
