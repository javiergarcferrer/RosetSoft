...
export default function CustomerChip({ customer, onOpen }) {
  if (!customer) {
    return (
      <button
        type="button"
        onClick={onOpen}
        className="inline-flex items-center gap-1.5 rounded-full border border-dashed border-ink-300 px-3 py-1 text-xs text-ink-500 hover:border-brand-400 hover:text-brand-700 hover:bg-brand-50/50 transition-colors active:scale-[0.98]"
      >
        <UserIcon size={12} />
        Asignar cliente
      </button>
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-ink-200 bg-white px-2.5 min-h-7 coarse:min-h-9 text-xs hover:border-ink-400 hover:bg-ink-50 transition-all active:scale-[0.98] max-w-full min-w-0 ring-1 ring-inset ring-black/5"
