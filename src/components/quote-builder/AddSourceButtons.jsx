...
export default function AddSourceButtons({ onOpenCatalog, onOpenInventory, variant = 'toolbar' }) {
  const cls = variant === 'cta'
    ? 'inline-flex items-center justify-center w-11 h-11 coarse:w-12 coarse:h-12 rounded-lg border border-ink-200 text-ink-700 hover:bg-ink-50 hover:border-ink-300 active:bg-ink-100 transition-colors'
    : 'inline-flex items-center justify-center w-9 h-9 coarse:w-10 coarse:h-10 rounded-md text-ink-600 hover:text-ink-900 hover:bg-ink-100 active:bg-ink-200 transition-colors';
  const size = variant === 'cta' ? 20 : 18;
  return (
