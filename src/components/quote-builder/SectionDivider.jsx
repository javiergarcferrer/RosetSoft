    ...
    >
      <div className="flex items-center gap-2">
        ...
        <DebouncedInput
          ...
          className="block flex-1 min-w-0 bg-transparent border-0 px-0 py-0 text-xs font-bold uppercase tracking-widest text-ink-600 placeholder:text-ink-400 placeholder:normal-case placeholder:font-medium placeholder:tracking-normal focus:outline-none"
        />
        {subtotalLabel && (
          <span className="text-xs font-semibold tabular-nums text-ink-500 whitespace-nowrap" title="Total de los artículos de esta sección">
            {subtotalLabel}
          </span>
        )}
        <button
          type="button"
          onClick={onRemove}
          className={`inline-flex items-center justify-center w-7 h-7 coarse:w-11 coarse:h-11 text-ink-300 hover:text-red-500 hover:bg-red-50 rounded-md transition-all active:scale-[0.92] flex-shrink-0 ${hover ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}
