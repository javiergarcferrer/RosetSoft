...
const MoneyInput = forwardRef(function MoneyInput({
  ...
  currency = 'USD',
  placeholder = '0',
  // Tailwind width utility for the outer wrapper. Default 7rem fits a
  // 6-digit amount comfortably; widen for ledger / total rows.
  widthClass = 'w-28',
  className = '',
  ...inputProps
}, ref) {
  ...
  return (
    <div className={`relative ${widthClass}`}>
      ...
      <DebouncedInput
        ...
        className={`w-full text-right tabular-nums input min-h-9 coarse:min-h-10 py-1.5 pl-6 pr-2 ${className}`}
        {...inputProps}
      />
    </div>
  );
